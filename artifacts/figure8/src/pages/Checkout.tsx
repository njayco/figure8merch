...
const formSchema = z.object({
  ...
  cardNumber: z.string().min(16, "Invalid card number (Test mode: use anything)").max(16),
  expiry: z.string().min(5, "MM/YY").max(5),
...
export function Checkout() {
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    const fullAddress = `${values.firstName} ${values.lastName}, ${values.address}, ${values.city}, ${values.state} ${values.zipCode}`;
    
    createOrder.mutate(
      { 
        data: { 
          shippingAddress: fullAddress,
          paymentMethodId: "pm_test_card" // Simulated Stripe
        } 
      },
      {
        onSuccess: (order) => {
